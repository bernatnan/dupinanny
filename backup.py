    ...
    def ProcessBackups(self):
        ...
        if (not self.dry_run and self.config.has_key('backup_every') and 
            ...
            if (delta.days < self.config['backup_every']):
                print('Last backup is %d days old, no new backup needed.' % 
                    delta.days)
                return

        if (not self.dupi.has_key('items')):
            raise Exception('no backups defined (\'items\' entry in the '
                'DupiConfig dictionary)')
...
class BackupTarget(object):
    ...
    def Setup(self, backup):
        self.backup = backup
        print 'BackupTarget.Setup %s' % repr((self.root, self.destination, 
    ...
    def Run(self, recursed=False):
        # we do it that way so a full backup that fails keeps getting retried 
        # until it works
        ...
        if (not self.backup.dry_run):
            ...
            while (ret is None):
                ...
                if (len(out) != 0):
                    ...
                    if (out.find('Old signatures not found and incremental '
                                'specified') != -1):
            ...
            if (ret != 0):
                if (failed_incremental):
                    ...
                    if (recursed):
                        raise Exception('already recursed while forcing full '
                            'backup')
                    subprocess.check_call(['touch', self.fullFileFlag])
        ...
        if (self.backup.remove_older != 0):
            cmd = [self.backup.duplicity, 'remove-older-than', '%dD' % 
...
class LVMBackupTarget(BackupTarget):
    def __init__(self, root, destination, lvmpath, snapsize, snapshot_name, 
        ...
        self.fullFileFlag = os.path.normpath('%s.full' % 
            lvmpath.replace('/', '_'))
